...
```{r}
df_world <- read.csv("~/Global-Analysis/Global_data.csv")
```
...
```{r}
df_world <- df_world %>%
  rename(c(Overall_Score = Overall.Score, 
...
scatter1 <- function(df, year) {
  df %>% 
    filter(Year == year) %>% 
    ggplot() + 
    geom_point(aes(x = Overall_Score, 
...
```{r, fig.width = 12, fig.height=9}
ggplot(df_world) +
  geom_density(aes(x = Overall_Score, 
                    color = as.character(Year)),
                show.legend = NA) +
...
df_world %>% 
  filter(Year == 2019) %>% 
  ggplot() + 
  geom_boxplot(aes(x = Continent, y = Overall_Score,
                   color = Continent)) +
...
Average schooling years across continents
